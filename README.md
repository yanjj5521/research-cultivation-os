...
- “设置与备份”新增可视化导航编排器：分组和组内条目都可拖拽排序，窄屏与触屏设备可使用上下箭头；
- 每个固定导航条目都能独立显示或隐藏；启用中的动态工作区入口也可作为一个整体开关；
- 隐藏“设置与备份”不会把自己锁在外面，侧栏底部始终保留“调整导航”安全入口；
- 布局保存为稳定键 JSON，升级时会去除未知或重复项，并把未来新增入口追加到默认分组，不破坏已有顺序与隐藏状态；
- 个性化包升级为 v7，导航名称、分组顺序、条目顺序和显示状态可导出、导入并通过 ResearchHub 轻量同步；
- `v2.3.0` 的生涯罗盘、法器购买、昼夜山门、真实联机和 Android 客户端全部保留。
...
- 导航根据本人的工作区动态生成；
- 工作区定义与导航布局进入 v7 个性化包；记录和附件仍进入完整备份与知识库导出。
...
- 网站名、39 阶境界名、导航名称与布局、诗句池、工作区定义、主题、主页信息和复盘开关可导出为 JSON；
- 个性化包不包含论文、数据、附件、API 密钥或联机 Token；
- 当前版本能导入 v1–v7 个性化包；v7 在 v6 的稳定境界键、压缩头像、每日诗句池与组合工作区基础上新增导航顺序和显示状态；
