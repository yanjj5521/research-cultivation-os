...
导入同时兼容 `research-cultivation-personalization-v1` 至 `v5`。v1.4 的 31 阶自定义名称会迁移到最接近的新稳定键；未修改的旧默认名自动替换为 v1.5 新名称。安全升级会迁移 `storage/profile/`，因此头像不会因版本更新丢失。
