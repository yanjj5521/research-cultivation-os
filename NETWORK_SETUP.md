...
v2.1.0 默认选择“未启用”，不会建立同步队列，也不会上传或拉取任何状态。规模化云端 v2 仍只是接口、事件信封和 OpenAPI 草案，服务器、统一登录与多租户数据库尚未实现。
...
3. 随机密码与 API Token 只写在中心主机的 `instance/HUB_ADMIN_CREDENTIALS.txt`；
