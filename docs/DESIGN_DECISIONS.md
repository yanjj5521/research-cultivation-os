...
本文件记录截至 2026-07-30 采用的外部依据与后续使用反馈。v2.3 在证据驱动的课题推进之上增加科研生涯连续性、时间化山门、可核算法器交易、可直接开启的 ResearchHub 通道与 Android 客户端；v2.3.1 进一步让导航顺序和显示状态由研究者本人决定。
...
W3C 的一致导航要求同一用户在一组页面中看到可预测的入口，而不是要求所有用户共享同一排序。v2.3.1 因此把“稳定身份”和“个人布局”分开：

- 页面、分组和工作区继续使用稳定键，改名、排序与隐藏不改 URL；
- 用户保存后的布局会在全部页面一致渲染；
- 分组与条目可拖拽，触屏或键盘操作可使用上下箭头；
- 隐藏状态只影响侧栏入口，不删除功能、数据或路由；
- 动态工作区入口作为整体开关，工作区内部仍使用现有启停与排序；
- 设置入口即使被隐藏，侧栏底部仍保留恢复链接；
- 新版本新增入口时，标准化器将其追加到默认分组，避免旧布局永远看不到新功能。

这使导航能够随科研阶段改变：实验密集期可以把实验与交付入口提前，写作期可以突出知识检索和课题记录，而不需要分叉源码或丢失历史配置。
