...
本文件记录 2026-07-27 开发时采用的外部依据与后续使用反馈。v2.0.1 保留科研闭环和游戏化底层规则，但把首页从高密度仪表盘收回为轻量山门。
...
v2.0 的实际使用反馈表明，把闭环的每一步、论文轮播、主动回忆、闪念、奖励和备份同时放入首页，会增加扫描与选择负担。v2.0.1 因此采用渐进呈现：首页只保留山门氛围、四个核心入口和本地/联网双检索，其余动作回到职责明确的独立页面。窄屏允许自然滚动，避免为了形式上的一屏裁掉内容。
