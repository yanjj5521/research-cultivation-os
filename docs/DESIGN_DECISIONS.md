...
本文件记录 2026-07-27 开发时采用的外部依据，避免功能只凭直觉堆砌。v2.0 重点新增习惯启动、内在动机、论文再遇见和单屏首页的设计边界。
...
- **自主**：用户可以在完整主线和“只做 10 分钟”之间选择；
- **胜任**：展示真实交付、固定奖励、本阶进度和最近七日节律；
- **意义**：论文卡、最小交付和能力里程碑都指向真实科研资产；
- **恢复**：漏一天不会抹掉近七天的行动，也不会生成补课债务。

“今日真实收获”只汇总已经写入活动与资材账本的奖励；未完成时不会显示虚假的随机宝箱。
...
v2.0 因此把首页固定为同一条行动顺序：

```text
看见唯一下一步 → 10 分钟启动 → 提交证据 → 次日温故
```

桌面首页在 1280×720 及更大尺寸用一屏呈现这条闭环；低频管理仍留在展开侧栏。首页同时显示论文卡、主动回忆、本地/联网双检索、闪念保存和备份入口，因为这些是用户每天真实使用的动作，不再把它们藏进二级菜单。窄屏允许自然滚动，避免为了形式上的“一屏”裁掉内容。

论文卡和每日一问按日期稳定轮换，同一天刷新不乱跳；联网检索只在用户点击时发生，首页加载不自动发送研究关键词。

## 可行动反馈，而非数据堆积

学习分析仪表盘研究强调，反馈需要帮助学习者理解目标并采取下一步行动，而不是只展示描述性数字。因此首页只保留会改变当日选择的指标：下一行动、最小交付、今日完成度、近七日节律、到期复习和下一境界距离。知识总数、成就全集和活动流水继续保留在相应页面，不占用首页注意力。
